...
class MyTCPServer(socketserver.BaseRequestHandler):
    def handle(self):
        try:
            while True:
                ...
                '''
                self.home_path = settings.HOMEPATH
                self.current_path = settings.HOMEPATH  # 登陆成功后立即定义一个“当前路径”变量，供后面创建目录，切换目录使用
                while True:
    ...
    def put(self, cmd):
        ...
        print("文件大小：", int(total_size), "剩余空间：", remain_size)
    ...
    def accountRemainSize(self):
        ...
        :return:
        """
        account_path = settings.HOMEPATH

        if platform.system() == 'Windows':
            free_bytes = ctypes.c_ulonglong(0)
            ctypes.windll.kernel32.GetDiskFreeSpaceExW(ctypes.c_wchar_p(account_path), None, None, ctypes.pointer(free_bytes))
            return free_bytes.value
        else:
            st = os.statvfs(account_path)
